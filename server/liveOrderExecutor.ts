...
export async function tryLiveEntry(params: LiveEntryParams): Promise<{ entered: boolean; reason: string; orderId?: string | null; sl?: number; tp?: number }> {
  ...
  // Hard cap: never more than $maxPositionUsd / $2 shares (defense against price cache bugs)
  const maxAllowedQty = Math.floor((config?.maxPositionUsd ?? 50000) / Math.max(aggressiveEntry, 2));
  const qty = Math.min(Math.max(1, rawQty), maxAllowedQty);
  if (rawQty > maxAllowedQty) {
    log.warn("LIVE_EXEC", `[QTY Cap] ${ticker} rawQty=${rawQty} capped to ${maxAllowedQty} (maxPositionUsd=${config?.maxPositionUsd})`);
  }

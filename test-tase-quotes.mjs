    ...
    const req = http.request(options, (res) => {
      ...
      res.on("end", () => {
        ...
        try {
          ...
          console.log("Quotes:", JSON.stringify(parsed.quotes?.map(q => ({ ticker: q.ticker, conid: q.conid, exchange: q.exchange_raw })), null, 2));
          console.log("Unresolved:", parsed.unresolved);
        } catch {
...
// Test with TASE tickers - with and without .TA suffix
console.log("=== Test 1: TASE tickers WITH .TA suffix ===");
await ibindReq("POST", "/quotes", { symbols: ["TRX.TA", "MNIF.TA", "KEN.TA"] });

console.log("\n=== Test 2: TASE tickers WITHOUT .TA suffix ===");
await ibindReq("POST", "/quotes", { symbols: ["TRX", "MNIF", "KEN"] });

console.log("\n=== Test 3: Mixed batch ===");
await ibindReq("POST", "/quotes", { symbols: ["AAPL", "MSFT", "TRX.TA"] });

...
export const liveEngineConfig = mysqlTable("liveEngineConfig", {
  ...
  // Dynamic VIP (Phase 1, 2026-07-01) — INERT BY DEFAULT. 1 = the war ENTER sort uses the daily
  // tier as a ≤0.5 finalScore tiebreak (rank ≠ gate bypass) + pyramid skips BENCH scale-ins.
  // benchAutoExitEnabled = Phase 2 (accelerated BENCH exit). Owner-only flip via War Room.
  dynamicVipEnabled:    tinyint("dynamicVipEnabled").notNull().default(0),
  benchAutoExitEnabled: tinyint("benchAutoExitEnabled").notNull().default(0),

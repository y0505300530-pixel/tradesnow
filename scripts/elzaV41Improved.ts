/**
 * ╔══════════════════════════════════════════════════════════════════════════╗
 * ║  elzaV41Improved.ts — Elza v4.1-Improved : Genesis-TRUE LONG-ONLY vs SPY.  ║
 * ║                                                                            ║
 * ║  PURE SIMULATION. NO DB writes. NO IBKR. NO orders. NO deploy. NO SSH.     ║
 * ║                                                                            ║
 * ║  "LET THE FAT TAIL RUN" — the Genesis-TRUE entry (faithful warEngine v1.0) ║
 * ║  with the fat-tail exit engine and NO FAST-KILL. The only time-stop is a   ║
 * ║  generous 60-bar backstop so a position can't hold forever; the Chandelier ║
 * ║  / structural SL exits almost everything well before then.                 ║
 * ║                                                                            ║
 * ║  ENTRY (identical to elzaV4Hybrid Config-1 "Genesis-TRUE"):                ║
 * ║    genesisScore (8 decimal sub-scores, cap +0.99, base up to 10);          ║
 * ║    gate = totalScore >= 7.0 AND confluence >= 4.5 AND liquidity >= 2.0     ║
 * ║    (getTickerIntelligence(ticker, bars.slice(0,i+1)) — REAL & CAUSAL).     ║
 * ║    EMA-200 macro guards baked into the tiers; RC-2 skip if risk > 12%.     ║
 * ║    SL = 10-bar low.                                                         ║
 * ║                                                                            ║
 * ║  EXIT (fat-tail): free-roll 50% at +1.5R → residual to breakeven →         ║
 * ║    Chandelier max(priorTrail, highHigh − 2.5×ATR14 simple). NO FAST-KILL.  ║
 * ║    60-bar backstop time-stop only. SL-FIRST-ON-TIE strict.                 ║
 * ║                                                                            ║
 * ║  FLIGHT RECORDER (CEO-mandated): per-trade log                             ║
 * ║    Ticker | EntryDate | ExitDate | MFE_R | FinalNetR | ExitReason          ║
 * ║    printed to stdout AND written to /tmp/elza-v41-tradelog-2026.md.        ║
 * ║                                                                            ║
 * ║  WINDOW: 2026 YTD ONLY. LONG-ONLY — NO SHORT ENGINE.                       ║
 * ╚══════════════════════════════════════════════════════════════════════════╝
 *
 * RUN (manager runs this on the droplet — has Yahoo/DB data; 149 tickers is slow):
 *   node --import tsx --env-file=.env scripts/elzaV41Improved.ts
 *
 * BUILD-CHECK (local, no run):
 *   npx esbuild scripts/elzaV41Improved.ts --bundle --platform=node --packages=external --outdir=/tmp/v41bc
 *
 * Scoring / exit / portfolio / dynamic-catalog / friction / conviction-ranking are
 * faithful copies of scripts/elzaV4Hybrid.ts Config-1 (Genesis-TRUE). Only `calcEMA`/
 * `calcRSI`/the `Bar` type (server/zivEngine.ts) and `getTickerIntelligence`
 * (server/runtimeIntelligence.ts) are reused — no P&L/price formula is re-implemented.
 */
...
// ── TRUE v1.0 ENTRY GATES (Genesis-TRUE — identical to elzaV4Hybrid Config-1). ──
...
// ── v4.1 "LET THE FAT TAIL RUN" TIME-STOP: generous 60-bar backstop ONLY. ──
//    NO FAST-KILL. The pre-free-roll backstop is widened from 20→60 bars so a
//    position can't hold forever, but Chandelier/SL exits almost everything first.
const GENESIS_TIME_STOP_BARS = 60;   // pre-free-roll backstop; residual trails with no time-stop
...
// ─── Window: 2026 YTD ONLY ──────────────────────────────────────────────────────
interface WindowDef { label: string; start: string; end: string | null; }
const WINDOW: WindowDef = { label: "W-2026", start: "2026-01-01", end: null };
const EARLIEST_WINDOW_START = WINDOW.start;

const TRADELOG_PATH = "/tmp/elza-v41-tradelog-2026.md";

type FrictionMode = "FRICTIONLESS" | "REALISTIC";

// ════════════════════════════════════════════════════════════════════════════
// GENESIS SCORING — copied verbatim from elzaV4Hybrid.ts (v1.0 spec).
// ════════════════════════════════════════════════════════════════════════════
...
// The forward exit walk is fat-tail (free-roll → BE → Chandelier), NO fast-kill,
// 60-bar backstop. Friction is applied later per-mode from the recorded fill LEVELS.
// ════════════════════════════════════════════════════════════════════════════
 ...
 * EXIT = fat-tail: free-roll +1.5R → BE → Chandelier; NO FAST-KILL; 60-bar backstop.
 * async because getTickerIntelligence is async (offline-causal: we pass
 * bars.slice(0, i+1) so it never fetches and is computed as-of the entry bar).
 *
 ...
 *
 * MFE_R (FLIGHT RECORDER): running max of (barHigh − entry)/(entry − initialSL) over
 * the WHOLE held window (both phases) — surfaced per trade for the CEO log.
 */
async function generateCandidates(
  ...
  for (; i < bars.length; i++) {
    ...
    // ── EXIT walk-forward — free-roll(1.5R) + Chandelier + 60-bar backstop. ──
    ...
    let mfeR = 0; // FLIGHT RECORDER — max favorable excursion in R over the WHOLE trade

    for (let j = i + 1; j < bars.length; j++) {
      const bar = bars[j];
      const heldBars = j - i;

      // track MFE in R over EVERY held bar (both phases) — favorable daily high only.
      ...
      if (!reachedFirstTarget) {
        ...
        // ── v4.1 generous 60-bar backstop (pre-free-roll only). NO FAST-KILL. ──
...
// ════════════════════════════════════════════════════════════════════════════
// PORTFOLIO SIM — CONVICTION-RANKED auction (copied from elzaV4Hybrid.ts).
// ════════════════════════════════════════════════════════════════════════════
...
// ─── SPY benchmark (copied from elzaV4Hybrid.ts) ──────────────────────────────
...
// FinalNetR | ExitReason), sorted by EntryDate. Printed to stdout AND written to
// /tmp/elza-v41-tradelog-2026.md. FinalNetR = REALISTIC (after-friction) R.
// ════════════════════════════════════════════════════════════════════════════
function buildFlightRecorder(realClosed: ClosedTrade[]): { stdout: string; markdown: string } {
  ...
  // ── markdown table for /tmp/elza-v41-tradelog-2026.md. ──
  const md: string[] = [];
  md.push(`# Elza v4.1-Improved — Flight Recorder (per closed trade) — 2026 YTD`);
  md.push("");
  md.push(`Generated ${new Date().toISOString()} — READ-ONLY simulation. LONG-ONLY. FinalNetR is REALISTIC (after 5bps/side + $1/side commission). MFE_R is daily-bar max favorable excursion in R (overstated vs intraday).`);
  md.push("");
  md.push(`| Ticker | EntryDate | ExitDate | MFE_R | FinalNetR | ExitReason |`);
  md.push(`|--------|-----------|----------|------:|----------:|------------|`);
  for (const t of rows) {
...
async function main(): Promise<void> {
  console.log(`Elza v4.1-Improved Backtest — Genesis-TRUE LONG-ONLY "let the fat tail run" (NO fast-kill) vs SPY — 2026 YTD — READ-ONLY (no DB writes, no IBKR, no orders, no SSH)`);
  ...
  console.log(`ENTRY GATES (Genesis-TRUE v1.0): totalScore >= ${LONG_ENTRY_MIN_SCORE} AND confluenceScore >= ${MIN_CONFLUENCE} AND liquidityScore >= ${MIN_LIQUIDITY_SCORE}.`);
  ...
  console.log(`RANKING (headline): per-day conviction auction — highest totalScore fills the ${MAX_CONCURRENT} LONG slots (NOT FIFO).`);
  console.log(`EXIT (fat-tail — LET WINNERS RUN): SL=${SL_LOOKBACK}-bar low; RC-2 skip if (entry-SL)/entry>${RC2_MAX_RISK_PCT}%; free-roll bank 50% at +${PARTIAL_TP1_R}R → BE; Chandelier max(prior, highHigh − ${CHANDELIER_ATR_MULT}×ATR${ATR_PERIOD}).`);
  console.log(`TIME-STOP: NO FAST-KILL. Generous ${GENESIS_TIME_STOP_BARS}-bar backstop (pre-free-roll only) so positions can't hold forever; Chandelier/SL exits almost everything first.`);
  console.log(`PESSIMISM: SL-FIRST-ON-TIE strict (stop wins any bar low<=stop & high>=target; +1.5R banks only if high>=TP & low>stop). REALISTIC = 5bps adverse entry & every exit fill + $1/side commission-in-R (headline); FRICTIONLESS for reference.`);
  console.log(`Portfolio: $${START_EQUITY.toLocaleString()}, ${RISK_PCT * 100}% risk, leverage ${LEVERAGE}×, maxConcurrent ${MAX_CONCURRENT} LONG, maxPerSector ${MAX_PER_SECTOR}, heat ≤ ${HEAT_CAP * 100}%, maxPos $${MAX_POSITION_USD.toLocaleString()}.`);
  console.log(`Window: W-2026 = entryDate in [2026-01-01, last bar]. SPY buy&hold 2026.`);
  console.log("");
  ...
  console.log(`[DONE BUILDING] ${cands.length} candidates (Genesis-TRUE, all bars).`);

  // SPY for the 2026 window.
  const spy = computeSpyBenchmark(WINDOW.label, spyBars, WINDOW.start, WINDOW.end);

  // ── Run the 2026 window (REALISTIC headline + FRICTIONLESS reference). ──
  ...
  // ════════════════════════════════════════════════════════════════════════
  // FLIGHT RECORDER — print + write file. (REALISTIC book; FinalNetR after friction.)
  // ════════════════════════════════════════════════════════════════════════
  const fr = buildFlightRecorder(real.closed);
  console.log("");
  console.log(`══════════════════════════════════════════════════════════════════════════════════════════`);
  console.log(`═══ FLIGHT RECORDER — per-trade log (Ticker | EntryDate | ExitDate | MFE_R | FinalNetR | ExitReason) — sorted by EntryDate ═══`);
  console.log(`══════════════════════════════════════════════════════════════════════════════════════════`);
  ...
  console.log("");
  console.log(`══════════════════════════════════════════════════════════════════════════════════════════`);
  console.log(`═══ v4.1-Improved — 2026 YTD SUMMARY — REALISTIC headline (LONG only, NO short engine) ═══`);
  console.log(`══════════════════════════════════════════════════════════════════════════════════════════`);
  console.log(`  ${pad("metric", 22)}${padL("value", 16)}`);
  console.log(`  ${"─".repeat(38)}`);
  console.log(`  ${pad("candidates-in-window", 22)}${padL(wc.length, 16)}`);
  console.log(`  ${pad("trades taken", 22)}${padL(real.tradesTaken, 16)}`);
  console.log(`  ${pad("win%", 22)}${padL(real.winPct.toFixed(1) + "%", 16)}`);
  console.log(`  ${pad("totalR", 22)}${padL(sign(real.totalR) + real.totalR.toFixed(2), 16)}`);
  console.log(`  ${pad("return% REALISTIC", 22)}${padL(sign(real.finalReturnPct) + real.finalReturnPct.toFixed(2) + "%", 16)}`);
  console.log(`  ${pad("return% frictionless", 22)}${padL(sign(fric.finalReturnPct) + fric.finalReturnPct.toFixed(2) + "%", 16)}`);
  console.log(`  ${pad("maxDD% REALISTIC", 22)}${padL(real.maxDrawdownPct.toFixed(2) + "%", 16)}`);
  console.log(`  ${pad("finalEq$ REALISTIC", 22)}${padL("$" + real.finalEquity.toLocaleString(), 16)}`);

  // Exit-reason breakdown — confirm FAST_TIME absent.
  console.log("");
  console.log(`  ── exit-reason breakdown (REALISTIC book) ──`);
  const eb = real.exitBreakdown;
  console.log(`     SL=${eb["SL"] ?? 0}  TIME(${GENESIS_TIME_STOP_BARS}bar)=${eb["TIME"] ?? 0}  TRAIL=${eb["TRAIL"] ?? 0}  TRAIL_OPEN=${eb["TRAIL_OPEN"] ?? 0}  OPEN=${eb["OPEN"] ?? 0}`);
  const fastTime = (eb as Record<string, number>)["FAST_TIME"] ?? 0;
  console.log(`     FAST_TIME (must be 0 — fast-kill REMOVED in v4.1): ${fastTime} ${fastTime === 0 ? "[ABSENT - OK]" : "[UNEXPECTED]"}`);

  // Catalog parity.
  console.log("");
  console.log(`  [CATALOG PARITY] live universe = ${TICKERS.length} USA tickers (expected ~149).`);

  // ── v4.1 vs SPY-2026 side-by-side + verdict. ──
  console.log("");
  console.log(`  ── v4.1-Improved vs SPY-2026 (REALISTIC) ──`);
  if (spy == null) {
    console.log(`     [SKIP] No SPY bars in 2026 window → SPY comparison INDETERMINATE.`);
    console.log(`     v4.1-Improved (realistic) = ${sign(real.finalReturnPct)}${real.finalReturnPct.toFixed(2)}%.`);
  } else {
    console.log(`     ${pad("strategy", 18)}${padL("return%", 12)}${padL("maxDD%", 12)}`);
    console.log(`     ${"─".repeat(42)}`);
    console.log(`     ${pad("v4.1-Improved", 18)}${padL(sign(real.finalReturnPct) + real.finalReturnPct.toFixed(2) + "%", 12)}${padL(real.maxDrawdownPct.toFixed(2) + "%", 12)}`);
    console.log(`     ${pad("SPY buy&hold", 18)}${padL(sign(spy.returnPct) + spy.returnPct.toFixed(2) + "%", 12)}${padL(spy.maxDDPct.toFixed(2) + "%", 12)}`);
    const diff = real.finalReturnPct - spy.returnPct;
    const verdict = diff >= 0 ? "BEATS" : "LOSES";
    console.log("");
    console.log(`     VERDICT: v4.1 ${verdict} SPY by ${Math.abs(diff).toFixed(2)} pts (realistic) ` +
      `[v4.1 ${sign(real.finalReturnPct)}${real.finalReturnPct.toFixed(2)}% vs SPY ${sign(spy.returnPct)}${spy.returnPct.toFixed(2)}%].`);
  }
  ...
  console.log(`[JSON] ${JSON.stringify({
    ...
    v41Improved: {
      ...
      realistic: {
        ...
        totalR: real.totalR,
        returnPct: Math.round(real.finalReturnPct * 100) / 100,
        maxDDPct: Math.round(real.maxDrawdownPct * 100) / 100,
        finalEquity: real.finalEquity,
        exitBreakdown: real.exitBreakdown,
      },
      frictionlessReturnPct: Math.round(fric.finalReturnPct * 100) / 100,
    },
  ...
  console.log("");
  console.log(`Done. 1 config (Genesis-TRUE) × 2026 YTD × 2 friction modes. (Simulation only — no live actions taken.)`);
}
...
function printDisclaimer(): void {
  ...
  console.log(`    Confluence/liquidity come from getTickerIntelligence(ticker, bars.slice(0,i+1)) — daily-bar-only & causal.`);
  console.log(`  • CONVICTION AUCTION is a per-CALENDAR-DAY ranking on totalScore; intraday entry timing is not modelled.`);
  console.log(`  • DAILY-BAR TRAIL/MFE OVERSTATES: +1.5R tag, Chandelier, and MFE_R use DAILY highs/lows; the fat-tail`);
  console.log(`    trail runs AND the recorded MFE_R are OVERSTATED vs intraday reality. Treat MFE_R as a ceiling.`);
  ...
  console.log(`  • SURVIVORSHIP: the universe is the CURRENT 2026 catalog (getCatalogTickers(1)); de-listed/removed names`);
  console.log(`    that traded in 2026 are not back-filled — a mild survivorship tilt.`);
  console.log("");

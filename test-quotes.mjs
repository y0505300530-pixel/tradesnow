...
function ibindReq(method, path, body) {
  return new Promise((resolve, reject) => {
    ...
    const req = http.request(options, (res) => {
      ...
      res.on("end", () => {
        console.log(`${method} ${path} => ${res.statusCode}: ${data.substring(0, 500)}`);
...
// /quotes returned 405 on GET — try POST
// Also try variations
const tests = [
  ["POST", "/quotes", { symbols: ["TRX"] }],
  ["POST", "/quotes", { symbols: "TRX" }],
  ["POST", "/quotes?symbols=TRX"],
  ["GET", "/api/quotes?symbols=TRX"],
  ["POST", "/api/quotes", { symbols: "TRX" }],
  ["GET", "/market/quotes?symbols=TRX"],
  ["POST", "/resolve-conid", { symbol: "TRX" }],
  ["POST", "/resolve-conid", { symbols: ["TRX"] }],
];
